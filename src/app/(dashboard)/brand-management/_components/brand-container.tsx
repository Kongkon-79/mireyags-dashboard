...
import BrandTableSkeleton from "./brand-table-skeleton";
...
export default function BrandsContainer() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<BrandsApiResponse>(
    {
      ...
      queryFn: async () => {
        ...
        if (!res.ok) {
          throw new Error("We couldn't load the brands right now. Please try again.");
        }
  ...
  if (isLoading) {
    tableContent = <BrandTableSkeleton />;
  } else if (isError) {
    tableContent = (
