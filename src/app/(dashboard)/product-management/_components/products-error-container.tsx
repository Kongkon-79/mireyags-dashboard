...
interface ProductsErrorContainerProps {
  message: string;
  onRetry: () => void;
}

export default function ProductsErrorContainer({
  message,
  onRetry,
}: ProductsErrorContainerProps) {
  return (
    <div className="flex min-h-[360px] w-full flex-col items-center justify-center rounded-[16px] border border-[#F3D4D4] bg-[#FFF7F7] px-6 py-10 text-center">
      ...
      <h3 className="mt-5 text-lg font-semibold text-[#242424]">
        Unable to load products
      </h3>
      <p className="mt-2 max-w-md text-sm leading-6 text-[#667085]">
        {message || "Something went wrong while loading the product list."}
      </p>
